            ...
            if rand_num <= edge_prob:
                ...
                for other_node in nodes:
                    if node != other_node:
                        ...
                        break
    return G, G.number_of_nodes(), G.number_of_edges()  # return the number of nodes and edges along with the graph
...
def compute_plot_data(params):
    shape, scale, max_edges_per_node = params 
    ratios = []
    graph_data = []
    node_counts = []
    edge_counts = []
    for i in range(5):
        G, node_count, edge_count = generate_graph(shape, scale, 1000)  # total_length is set to 1000
    ...
    return shape, scale, avg_ratio, std_dev_ratio, avg_nodes, avg_edges, graph_data  # return average nodes and edges
...
if __name__ == '__main__':
    ...
    with open('network_plot_data_kk_v4.pkl', 'wb') as f:
